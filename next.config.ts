...
const nextConfig: NextConfig = {
  // Per-user build cache. If two OS accounts share one checkout, a single .next
  // hits cross-user permission errors ("Operation not permitted") and crashes the
  // dev server. Point NEXT_DIST_DIR at a per-account dir from each shell profile
  // to avoid it. Default stays .next.
  ...
  async redirects() {
    return [
      { source: '/styleguide', destination: '/auis/styleguide', permanent: false },
      { source: '/styleguide/:path*', destination: '/auis/styleguide/:path*', permanent: false },
    ];
  },
};
