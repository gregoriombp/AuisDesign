...
The CLI unpacks the template, renames the project, initializes git, and installs dependencies (postinstall generates `.claude/skills` + `.agents/skills`). `--pm pnpm`, `--ref <branch|tag>`, `--no-install`, `--no-git` and `--force` are there when you need them — see [`packages/cli`](../packages/cli).
