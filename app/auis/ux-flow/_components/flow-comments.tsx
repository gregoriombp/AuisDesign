...
import { formatRelative } from "@/lib/auis-review/format"
import { makeId } from "@/components/auis-review/storage/utils"
import { REVIEW_PALETTE, SCHEMA_VERSION } from "@/components/auis-review/constants"
import type { ReviewComment, ReviewPoint } from "@/components/auis-review/types"
...
export function CommentNode({ data }: NodeProps<Node<CommentNodeData>>) {
  ...
  return (
    <div className="relative cursor-pointer select-none" title={c.text}>
      ...
      >
        <span className="text-3xs font-bold leading-none text-white">
          {c.authorName.charAt(0).toUpperCase()}
        </span>
        {replyCount > 0 && (
          <span className="text-3xs font-semibold leading-none text-white/90">
            {replyCount}
          </span>
        )}
      </div>
      {inReview && (
        <span className="absolute -right-1 -top-1 h-2.5 w-2.5 rounded-full bg-(--au-amber-500) ring-2 ring-(--bg-canvas)" />
      )}
    </div>
  )
}
  ...
  const submit = async () => {
    ...
    try {
      ...
      if (!id) {
        ...
        await setIdentity(nm, REVIEW_PALETTE[3]?.token ?? "var(--au-purple-600)")
    ...
    <PopoverShell at={at}>
      <div className="flex items-center gap-2 border-b border-(--border-subtle) px-3 py-2">
        ...
        {comment.status === "in_review" && (
          <span className="rounded-xs bg-(--au-amber-100) px-1.5 py-0.5 text-3xs font-medium text-(--au-amber-800)">
            in review
          </span>
        )}
        <button
          ...
          <div className="mt-2 flex flex-col gap-2 border-t border-(--border-subtle) pt-2">
            {replies.map((r) => (
              <div key={r.id} className="flex items-start gap-2">
                <span
                  className="mt-0.5 flex h-4 w-4 shrink-0 items-center justify-center rounded-full text-3xs font-semibold text-white"
                ...
                <div className="min-w-0">
                  <div className="flex items-center gap-1.5">
                    <span className="text-2xs font-medium text-(--fg-primary)">
                      {r.authorName}
                    </span>
                    {r.authorKind === "agent" && (
                      <span className="rounded-xs bg-(--bg-muted) px-1 text-3xs text-(--fg-tertiary)">
                        agent
                      </span>
                    )}
                  ...
                  <p className="m-0 whitespace-pre-wrap text-sm leading-snug text-(--fg-primary)">
                    {r.text}
                  </p>
                </div>
              </div>
