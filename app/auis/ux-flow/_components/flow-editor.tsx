 ...
 * always works, and the executor agent reads the suggestions straight from the repo file.
 * ──────────────────────────────────────────────────────────────────── */
  ...
  return (
    <Popover onOpenChange={(open) => { if (open) setVal(href) }}>
      ...
      <PopoverContent
        ...
        onPointerDown={(e) => e.stopPropagation()}
      >
        <label className="text-2xs font-medium text-(--fg-secondary)">Link (prototype route)</label>
        <input
          value={val}
  ...
  return (
    <div className={`group relative w-[184px] h-[150px] ${cursor}`}>
      ...
      <div className={`absolute inset-0 flex flex-col items-center justify-center gap-0.5 px-6 text-center ${editing ? "" : "pointer-events-none"}`}>
        ...
        <InlineText value={data.title} mode={mode} onCommit={(v) => onUpdateNodeData(id, { title: v })} placeholder="Flow name" className="text-sm font-semibold leading-tight text-(--au-purple-900)" inputClassName="text-sm font-semibold text-(--au-purple-900) text-center" autoEdit={autoEditId === id} />
      </div>
...
function useFlowSuggestions(flow: string) {
  ...
  const create = useCallback(
    async (
      ...
      try {
        const proposed = cleanForPrompt(nodes, edges)
        const base = cleanForPrompt(baseNodes, baseEdges)
...
function StatusPill({ status }: { status: SuggestionStatus }) {
  const map: Record<SuggestionStatus, { label: string; variant: "warning" | "info" | "live" | "neutral" }> = {
    open: { label: "Open", variant: "warning" },
    in_review: { label: "In review", variant: "info" },
    applied: { label: "Applied", variant: "live" },
    discarded: { label: "Discarded", variant: "neutral" },
  }
  const s = map[status]
  return <AuPill variant={s.variant} dot={false}>{s.label}</AuPill>
}
...
const NEW_CROSSFLOW_DATA: ScreenData = {
  ...
  title: "Other flow",
  href: "/auis/ux-flow/",
  note: "Jump to another flow in the hub.",
}
...
function autoLayout(nodes: Node[], edges: Edge[]): Node[] {
  ...
  while (queue.length > 0) {
    ...
    // First visit wins. Re-enqueuing a node at a deeper level made cycles grow
    // forever (A → B → A kept replacing 0 with 2, 4, 6…). Breadth-first
    // traversal already reaches every node at its shallowest deterministic
    // level, so revisits are both unnecessary and unsafe.
...
// JSON stays focused on the structural fields the executor agent needs.
...
export function FlowDiagram({
  ...
  const toggleSubflow = useCallback(
    async (diamond: Node) => {
      ...
      if (!isExpandableFlow(slug)) {
        if (data.href && data.href !== "#") setConfirmFlow({ open: true, target: { title: data.title, href: data.href } })
        return
      }
      ...
      const expansion = buildExpansion(diamond, loaded, data.title, {
        onOpen: () => setConfirmFlow({ open: true, target: { title: data.title, href: data.href } }),
  ...
  return (
    <FlowEditorContext.Provider value={ctxValue}>
          ...
          >
            ...
            {!editMode && !previewSugg && suggestions.length > 0 && (
              <Panel position="top-right">
                ...
                >
                  <span className="inline-flex items-center justify-center w-4 h-4 rounded-full bg-(--au-amber-500) text-white text-3xs font-bold">
                    {suggestions.length}
                  </span>
            ...
            {editMode && (
              <Panel position="bottom-center" className="bottom-16!">
                <div className="flex items-center gap-1.5 rounded-full border border-(--border-default) bg-(--bg-raised) px-2 py-1.5 text-xs shadow-(--shadow-md)">
                  ...
                  <button onClick={undo} disabled={!canUndo} title="Undo (⌘Z)" aria-label="Undo" className="rounded-full px-2 py-1 text-(--fg-secondary) transition hover:bg-(--bg-muted) hover:text-(--fg-primary) disabled:cursor-not-allowed disabled:opacity-30 disabled:hover:bg-transparent"><Icon name="undo" size={16} /></button>
                  <button onClick={redo} disabled={!canRedo} title="Redo (⌘⇧Z)" aria-label="Redo" className="rounded-full px-2 py-1 text-(--fg-secondary) transition hover:bg-(--bg-muted) hover:text-(--fg-primary) disabled:cursor-not-allowed disabled:opacity-30 disabled:hover:bg-transparent"><Icon name="redo" size={16} /></button>
            ...
            {previewSugg && (
              <Panel position="top-center">
                <div className="flex items-center gap-3 bg-(--bg-raised) border border-(--border-default) rounded-md px-4 py-2 text-sm shadow-(--shadow-md)">
                  ...
                  <code className="text-3xs font-mono px-1.5 py-0.5 rounded-sm bg-(--bg-muted) text-(--fg-secondary)">{previewSugg.id}</code>
      ...
      <AuModal
        open={showSave}
        onClose={() => { if (!saving) setShowSave(false) }}
        dismissible={!saving}
        size="sm"
        ...
        footer={
          <div className="flex w-full items-center justify-between gap-2">
            <AuButton
              ...
              disabled={saving}
            >
              Copy a prompt for the chat
            </AuButton>
            <div className="flex gap-2">
              <AuButton variant="secondary" size="sm" onClick={() => setShowSave(false)} disabled={saving}>
                Cancel
              </AuButton>
              <AuButton variant="primary" size="sm" onClick={confirmSave} disabled={!desc.trim()} loading={saving}>
                Save
              </AuButton>
            </div>
          </div>
      ...
      >
        <div className="flex flex-col gap-3">
          <p className="m-0 text-sm text-(--fg-secondary)">Describe what changes in this flow. The agent reads this to understand the intent behind the edit.</p>
          <AuTextarea
      ...
      <AuModal
        ...
        size="lg"
        title="Suggestions"
        titleAdornment={<AuPill variant="warning" dot={false}>{suggestions.length}</AuPill>}
        className="max-h-[80vh]"
      >
        <div className="flex min-h-0 flex-col gap-4 overflow-hidden">
          <p className="m-0 text-xs text-(--fg-secondary)">
            Suggestions are saved in the repo (<code className="font-mono text-2xs">flow-bridge/data/suggestions.json</code>). Send the ID to the agent in the chat (<em>&quot;evaluate suggestion X of flow {flow}&quot;</em>) and it materializes the proposal with a receipt. Approving archives it; rejecting sends it back to open.
          </p>
              ...
              {suggestions.map((s) => (
                <li key={s.id} className="rounded-md border border-(--border-subtle) bg-(--bg-canvas) p-4 flex flex-col gap-2">
                  <div className="flex items-baseline gap-2 flex-wrap">
                    <code className="rounded-sm bg-(--bg-muted) px-1.5 py-0.5 font-mono text-2xs text-(--fg-secondary)">{s.id}</code>
                  ...
                  {!s.baseRevision && (
                    <p className="m-0 caption text-(--fg-tertiary)">
                      Legacy suggestion: still readable, but it has to be recreated on top of the current revision before it can be materialized.
                    </p>
                  )}
                  {s.materializationReceipt && (
                    <p className="m-0 caption text-(--fg-tertiary)">
                      {s.materializationReceipt.summary} · {s.materializationReceipt.validations.length} validation(s)
                    </p>
                  )}
                  <div className="mt-1 flex flex-wrap gap-2">
                    <AuButton variant="primary" size="sm" onClick={() => viewSugg(s)}>View</AuButton>
                    <AuButton
                      variant="secondary"
                      size="sm"
                    ...
                    {s.status === "in_review" && (
                      <>
                        <AuButton
                          variant="primary"
                          size="sm"
                          disabled={!s.materializationReceipt}
      ...
      <FlowConfirmModal
        open={confirmFlow.open}
        target={confirmFlow.target}
        onCancel={() => setConfirmFlow((current) => ({ ...current, open: false }))}
        onConfirm={() => {
          if (!confirmFlow.target) return
          const href = confirmFlow.target.href
          setConfirmFlow((current) => ({ ...current, open: false }))
          router.push(href)
        }}
      />
...
function FlowConfirmModal({
  ...
  return (
    <AuModal
      ...
      onClose={onCancel}
      size="sm"
      title="Go to another flow?"
      footer={
        target ? (
          <>
            <AuButton variant="secondary" size="sm" onClick={onCancel}>
              Cancel
            </AuButton>
            <AuButton variant="primary" size="sm" iconRight="arrow_forward" onClick={onConfirm}>
              Go to {target.title}
            </AuButton>
          </>
        ) : undefined
      ...
      {target && (
        <div className="flex items-start gap-3.5">
          ...
          <p className="m-0 text-sm leading-relaxed text-(--fg-secondary)">
            ...
            <b className="font-medium text-(--fg-primary)">{target.title}</b> flow. You can come back at any time.
          </p>
        </div>
      )}
    </AuModal>
 ...
 * NODES/EDGES embedded as JSON. Two modes: ask the executor agent to apply the
...
function CopyPromptModal({
  ...
  const prompt = useMemo(() => {
    ...
    if (mode === "flow-update") {
      return [
        `Please update the \`${flow}\` flow in the UX Flow hub with the structure below.`,
        ``,
        `File: \`app/auis/ux-flow/${flow}/page.tsx\``,
  ...
  return (
    <AuModal
      ...
      onClose={onClose}
      size="lg"
      title="Copy a prompt for the agent"
      className="max-h-[85vh] overflow-hidden"
      footer={
        <>
          <AuButton variant="secondary" size="sm" onClick={onClose}>
            Close
          </AuButton>
          <AuButton variant="primary" size="sm" iconLeft="content_copy" onClick={copy}>
    ...
    >
      <div className="flex min-h-0 flex-col gap-4 overflow-y-auto">
        <p className="m-0 text-sm leading-relaxed text-(--fg-secondary)">
          Paste it into the chat with the agent. The node and edge JSON goes inside the prompt.
        </p>

        <div className="flex flex-col gap-2">
          <span className="text-xs font-medium text-(--fg-secondary) uppercase tracking-wide">What should the agent do?</span>
          <AuRadioGroup
            value={mode}
            onChange={(next) => setMode(next as "flow-update" | "product-routes")}
            label="What should the agent do?"
            className="flex flex-col gap-1.5"
          >
            <label
              htmlFor="prompt-mode-flow-update"
              className={
                mode === "flow-update"
            ...
            >
              <AuRadioGroupItem
                id="prompt-mode-flow-update"
                value="flow-update"
                className="mt-0.5"
              />
              <span className="flex flex-col gap-0.5">
                <span className="text-sm font-medium text-(--fg-primary)">Update this flow in the hub</span>
                <span className="text-xs text-(--fg-secondary) leading-snug">
                  Replaces <code className="rounded bg-(--bg-muted) px-1 py-0.5 font-mono text-2xs">NODES</code> and <code className="rounded bg-(--bg-muted) px-1 py-0.5 font-mono text-2xs">EDGES</code> in this flow&apos;s <code className="rounded bg-(--bg-muted) px-1 py-0.5 font-mono text-2xs">page.tsx</code> and adds an entry to <code className="rounded bg-(--bg-muted) px-1 py-0.5 font-mono text-2xs">updates</code>.
                </span>
              </span>
            </label>

            <label
              htmlFor="prompt-mode-product-routes"
              className={
                mode === "product-routes"
                  ? "flex items-start gap-2.5 cursor-pointer p-3 rounded-md border border-(--au-blue-400) bg-(--au-blue-100) transition"
                  : "flex items-start gap-2.5 cursor-pointer p-3 rounded-md border border-(--border-default) hover:bg-(--bg-muted) transition"
            ...
            >
              ...
              <span className="flex flex-col gap-0.5">
                ...
                <span className="text-xs text-(--fg-secondary) leading-snug">
                  Scaffolds <code className="rounded bg-(--bg-muted) px-1 py-0.5 font-mono text-2xs">app/&lt;href&gt;/page.tsx</code> for each screen. Decisions don&apos;t become pages.
                </span>
              </span>
            </label>
          </AuRadioGroup>
        ...
        <details>
          ...
          <pre className="mt-2 max-h-48 overflow-auto whitespace-pre-wrap wrap-break-word rounded-sm border border-(--border-subtle) bg-(--bg-canvas) p-3 font-mono text-2xs leading-relaxed text-(--fg-secondary)">
            {prompt}
          </pre>
        </details>
      </div>
    ...
    >
      {!hasPrototype ? (
        ...
        <div className="flex flex-col items-center justify-center text-center gap-3 py-12">
          ...
          <p className="m-0 text-sm text-(--fg-secondary) max-w-sm leading-relaxed">
            This step points to an address outside the platform. Open it in a new tab to see it.
          </p>
          <a
            href={href}
            target="_blank"
